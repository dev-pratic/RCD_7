...
//      $this->error( 'Invalid cell index ' . $cell );
    ...
    /**
 * Extrai comentários das células de uma planilha
 * 
 * @param int $worksheetIndex Índice da planilha (0-based)
 * @return array Array associativo com coordenadas da célula como chave e comentário como valor
 */
public function getComments($worksheetIndex = 0)
{
    $comments = [];
    ...
    // Constrói o caminho para o arquivo de comentários
    $sheetFile = $this->sheetFiles[$worksheetIndex];
    $commentsFile = str_replace('worksheets/sheet', 'comments', $sheetFile);
    $commentsFile = str_replace('.xml', '.xml', $commentsFile);
    
    // Tenta encontrar o arquivo de comentários correspondente
    if (!$this->entryExists($commentsFile)) {
        // Tenta outro padrão comum: xl/comments{id}.xml
        $commentsId = $worksheetIndex + 1;
        $commentsFile = 'xl/comments' . $commentsId . '.xml';
        
        if (!$this->entryExists($commentsFile)) {
            return $comments; // Sem comentários nesta planilha
        }
    }
    
    // Carrega o XML de comentários
    $commentsXML = $this->getEntryXML($commentsFile);
    if (!$commentsXML) {
        return $comments;
    }
    
    // Processa os comentários
    if (isset($commentsXML->commentList->comment)) {
        foreach ($commentsXML->commentList->comment as $comment) {
            $cellRef = (string)$comment['ref'];
            
            // Extrai o texto do comentário
            $text = '';
            if (isset($comment->text)) {
                if (isset($comment->text->t)) {
                    // Texto simples
                    $text = (string)$comment->text->t;
                } elseif (isset($comment->text->r)) {
            ...
            // Se houver autor, pode incluí-lo também
            $author = '';
            if (isset($comment['authorId']) && isset($commentsXML->authors->author)) {
                $authorId = (int)$comment['authorId'];
                if (isset($commentsXML->authors->author[$authorId])) {
                    $author = (string)$commentsXML->authors->author[$authorId];
                }
            }
...
/**
 * Extrai comentários de todas as células de uma planilha em formato simplificado
 * 
 * @param int $worksheetIndex Índice da planilha (0-based)
 * @return array Array com coordenadas da célula e texto do comentário
 */
public function getCommentsSimple($worksheetIndex = 0)
{
    $comments = $this->getComments($worksheetIndex);
    $simpleComments = [];
    
    foreach ($comments as $cellRef => $comment) {
        $simpleComments[$cellRef] = $comment['text'];
    }
...
/**
 * Verifica se uma célula específica tem comentário
 * 
 * @param int $worksheetIndex Índice da planilha (0-based)
 * @param string $cellRef Referência da célula (ex: 'A1', 'B5')
 * @return string|false Retorna o comentário ou false se não existir
 */
public function getCellComment($worksheetIndex = 0, $cellRef = 'A1')
{
    $comments = $this->getComments($worksheetIndex);
    
    if (isset($comments[$cellRef])) {
        return $comments[$cellRef]['text'];
    }
...
/**
 * Extrai comentários incluindo informações de formatação rich text
 * 
 * @param int $worksheetIndex Índice da planilha (0-based)
 * @return array Comentários com formatação
 */
public function getCommentsRich($worksheetIndex = 0)
{
    $comments = [];
    ...
    $authors = [];
    if (isset($commentsXML->authors->author)) {
        foreach ($commentsXML->authors->author as $author) {
    ...
    if (isset($commentsXML->commentList->comment)) {
        foreach ($commentsXML->commentList->comment as $comment) {
            $cellRef = (string)$comment['ref'];
            $authorId = isset($comment['authorId']) ? (int)$comment['authorId'] : null;
            
            $commentData = [
                'ref' => $cellRef,
                'author' => ($authorId !== null && isset($authors[$authorId])) ? $authors[$authorId] : '',
                'text' => '',
                'richText' => []
            ];
            ...
            if (isset($comment->text)) {
                if (isset($comment->text->t)) {
                    $commentData['text'] = (string)$comment->text->t;
                    $commentData['richText'][] = [
                        'text' => (string)$comment->text->t,
                        'format' => null
                    ];
                } elseif (isset($comment->text->r)) {
                    foreach ($comment->text->r as $run) {
                        $runData = ['text' => (string)$run->t];
                        
                        if (isset($run->rPr)) {
                            $format = [];
                            if (isset($run->rPr->b)) $format['bold'] = true;
                            if (isset($run->rPr->i)) $format['italic'] = true;
                            if (isset($run->rPr->u)) $format['underline'] = true;
                            if (isset($run->rPr->strike)) $format['strike'] = true;
                            if (isset($run->rPr->sz)) $format['size'] = (string)$run->rPr->sz['val'];
                            if (isset($run->rPr->color)) $format['color'] = (string)$run->rPr->color['rgb'];
                            if (isset($run->rPr->rFont)) $format['font'] = (string)$run->rPr->rFont['val'];
                            
                            $runData['format'] = $format;
                        }
                        
                        $commentData['richText'][] = $runData;
                        $commentData['text'] .= (string)$run->t;
                    }
                }
            }
            
            $comments[$cellRef] = $commentData;
        }
    }
