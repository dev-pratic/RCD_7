...
class javaExe extends generic {
	...
	function __construct($inPage=false,$father=false,$name=false,$nivel=1){
		if($inPage===true){			
			if($name===false){
				$name = "profiableJavaExe_".time()."_".rand(100,999);
			}
	...
	public function closeBaseCarregando($parent=true){
		global $page;
	...
	public function closeCarregando($parent=true){
		$this->closeBaseCarregando($parent);
	}
	...
	public function showMsg($parent=false,$obj="",$msg="",$pathCaseYes="",$pathCaseNo="",$typeButton="n",$backGroundColor=COLOR_ULTRA_DARK_RED,$id=false,$functionGoToPage=false){
		global $page;
		...
		$e = "
		<script language=\"javascript\">"
			.($functionGoToPage === true ? "
				function goToPage(Page,Parameter,Value,Target,addParameter,Child,http){
					if(Target==''){
						openBaseCarregando();
					}
					if(Child!=''){
						if(Parameter!=''){
							document.getElementById(Child).src = Page + '?' + Parameter + '=' + Value + addParameter;
						}else{
							document.getElementById(Child).src = Page;
						}
					}else if(Target!='' && Target!='_parent' && Target!='_PARENT'){
						if(http==''){
							http = 'http';
						}
						if(Parameter!=''){
							window.open(http + '://' + Page + '?' + Parameter + '=' + Value + addParameter,'_blank');
						}else{
							window.open(http + '://' + Page,'_blank');
						}
					}else if(Target=='_parent' || Target=='_PARENT'){
						if(Parameter!=''){
							window.parent.location.href = Page + '?' + Parameter + '=' + Value + addParameter;
						}else{
							window.parent.location.href = Page;
						}
					}else{
						if(Parameter!=''){
							document.location.href = Page + '?' + Parameter + '=' + Value + addParameter;
						}else{
			...
			function showMsgEvent".$obj.$id."(){
				...
				".($pathCaseYes!=$pathCaseNo ? $parent."document.getElementById(\"btNo\").style.display = \"block\";" : $parent."document.getElementById(\"btNo\").style.display = \"none\";")."
		...
		if($this->stringToUpper($typeButton)=="N"){
			$e .= "
