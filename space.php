...
class space extends generic {
	...
	public function End(){
		...
		if($this->panel===true){
			$this->e("<div id=\"base".$this->name."\" name=\"base".$this->name."\" class=\"container-fluid basePanel\" style=\"".$this->invisiblePanel."\" align=\"center\">",$this->nivel);
		}
	...
	public function includeFile($path){
		$this->inSide(file_get_contents($path));
	}
	public function inSide($text,$tab=0,$tabStruct=false){
		for($i=1;$i<=$tab;$i++){
			if($tabStruct===false){
				$text = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$text;
			}else{
