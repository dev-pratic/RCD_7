<?php
# New Class Table
class newTable extends generic {
	private $father;
	...
	function __construct($father,$name,$subLayer=0,$nivel=0){
		...
		$this->newObj($this,$this->name,"newTable",$this->father->getName());
	}
	...
	public function line($class="",$width=false){
	...
	public function cell($class=false,$width=false,$align=false,$valign=false,$styles=false,$java=false){
		...
		if($this->withoutCell===false){
			$id = count($this->linesHtml);
			$this->linesHtml[$id]['html'] = "<td id=\"C".$this->name.$id."\" name=\"C".$this->name.$id."\" class=\"".$class."\"".$width.$align.$valign.$styles.$java.">";
			$this->linesHtml[$id]['nivel'] = $this->nivelInterno++;
			$this->openCell = true;
		}
	}
	...
	public function inSideBold($text=""){
		$id = count($this->linesHtml);
	...
	public function comboBox($name,$column=false,$selectedIndex="",$class="textBox comboBox",$multiple=false,$disabled=false,$readOnly=false,$styles=false){
		$this->comboBox[$name]['name'] = $name;
		$this->comboBox[$name]['column'] = $column;
		$this->comboBox[$name]['selectedIndex'] = $selectedIndex;
		$this->comboBox[$name]['class'] = $class;
		if($multiple===true){
	...
	public function textBox($name,$column=false,$class="textBox",$type="text",$value=false,$maxlen=false,$disabled=false,$readOnly=false,$styles=false){
		$this->textBox[$name]['name'] = $name;
		$this->textBox[$name]['column'] = $column;
		$this->textBox[$name]['class'] = $class;
	...
	public function textBoxMultiLine($name,$column=false,$class="textBox",$value=false,$maxlen=false,$disabled=false,$readOnly=false,$styles=false){
		$this->textBoxMultiLine[$name]['name'] = $name;
		$this->textBoxMultiLine[$name]['column'] = $column;
		$this->textBoxMultiLine[$name]['class'] = $class;
	...
	public function button($name,$value,$class="bigButton",$disabled=false,$styles=false,$createLine=false){
		$this->button[$name]['name'] = $name;
		$this->button[$name]['value'] = $value;
		$this->button[$name]['class'] = $class;
		...
		foreach ($this->linesHtml as $key => $value){
			if(strpos($this->linesHtml[$key]['html'],"@obj#")!==false){
				...
				if($typeObj=="comboBox"){
					...
					$this->e("<select id=\"".$nameObj."\" name=\"".$nameObj.($this->comboBox[$nameObj]['multiple'] ? "[]" : "")."\" class=\"".$this->comboBox[$nameObj]['class']."\"".$this->comboBox[$nameObj]['multiple'].$this->comboBox[$nameObj]['disabled'].$this->comboBox[$nameObj]['readOnly']." style=\"".$this->comboBox[$nameObj]['styles']."\">",$this->linesHtml[$key]['nivel']);
					foreach ($this->optionComboBox as $keyB => $valueB){
						if($this->optionComboBox[$keyB]['nameComboBox']==$nameObj){
							$selected = "";
							if($this->optionComboBox[$keyB]['value']==$this->comboBox[$nameObj]['selectedIndex']){ 
								$selected = " selected"; 
							}
				...
				if($typeObj=="button"){
					...
					$this->e($br."<div id=\"".$this->button[$nameObj]['name']."\" name=\"".$this->button[$nameObj]['name']."\" class=\"".$this->button[$nameObj]['class']."\" align=\"center\" style=\"".$this->button[$nameObj]['styles']."\">".$this->button[$nameObj]['value']."</div>",$this->linesHtml[$key]['nivel']);
				}
				if($typeObj=="video"){	
					global $vW;
					...
					//<script type=\"text/javascript\" src=\"".$PATH."../../RCD_7/instascan.min.js\"></script>
					$this->e("
