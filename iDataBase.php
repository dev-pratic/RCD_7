...
class iDataBase extends generic {
	...
	public function connect(){
		if($this->sqlServer===false){
			...
			if($this->connected===false){
				$this->sqlServerConn = sqlsrv_connect($this->server.($this->port===false ? "" : ", ".$this->port),array("Database" => $this->dataBase, "UID" => $this->user, "PWD" => $this->pass, "CharacterSet" => "UTF-8"));
		...
		if($this->sqlServer===false){
			...
			if($this->sqlServerConn===false){
				...
				if($VERSION_PHP=="7.2"){
					...
					@$this->res = sqlsrv_query($this->sqlServerConn, $this->sql, array() ,array( "Scrollable" => $scrollable));//SQLSRV_CURSOR_KEYSET
				}else{
					...
					try{
						if(stripos($this->sql,"UPDATE")!==FALSE || stripos($this->sql,"INSERT")!==FALSE){
							$this->res = sqlsrv_query($this->sqlServerConn, $this->sql, null ,array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
						}else{
							$this->res = sqlsrv_query($this->sqlServerConn, $this->sql, null ,array( "Scrollable" => $scrollable, 'SendStreamParamsAtExec' => true));//SQLSRV_CURSOR_STATIC
						}
					}catch(Exception $e){
						$this->res = false;
					}
				...
				if($this->res===false){
					$errors = sqlsrv_errors();
					foreach ($errors as $error){
						$this->setError("(".$error['code'].") ".$error['message']."<br><br>{ ".$this->sql." }");
					}
					return false;					
				}else{
					return $this->res;
	...
	public function getError($n=false,$access=false,$all=false){
		...
		if(($REPORTING_ERROR==1 && $page->session->getLoginId()==1) || $page->session->getLoginTipo()==2 || $access===true || $_ENVIRONMENT===true){
			if($n===false && $all===false){
				$n = count($this->error);
				...
				$temp = "";
				$t = count($this->error);
				foreach($this->error as $kB => $vB){
					$temp .= str_replace("@numError", $kB."/".$t, $this->error[$kB])."<br>";
				}
				return $temp;
			}
		}else{
	...
	public function setConnection($server,$user,$pass,$dataBase=false,$port=false){
		...
		if($this->seg===false){
			...
			$this->serverSeg = $server;
			$this->userSeg = $user;
			$this->passSeg = $pass;
			$this->dataBaseSeg = $dataBase;
			$this->portSeg = $port;
		}
	}
	...
	public function restoreConnection($connection){
					...
					if(isset($connection['pass'])){
						if(isset($connection['database'])){
							if(isset($connection['port'])){
								$this->setConnection($connection['server'],$connection['user'],$connection['pass'],$connection['database'],$connection['port']);
							}
						}
					}
				}
	...
	public function restoreSeg(){
		if($this->seg===true){
			$this->server = $this->serverSeg;
			$this->user = $this->userSeg;
			$this->pass = $this->passSeg;
			$this->dataBase = $this->dataBaseSeg;
			$this->port = $this->portSeg;
		}
	}
