...
class head extends generic {
	...
	public function linkCss($address,$withoutPath=false,$before=false){
		$id = count($this->link);
		if($before===false){
			$before = 0;
		}else{
		...
		if($withoutPath===false){
			$this->link[$id]['address'] = $this->path.$address;
		}else{
			$this->link[$id]['address'] = $address;
		}
		$this->link[$id]['before'] = $before;
	}
	...
	public function End(){
        ...
        $this->e("<head>");
		if($this->ico){
			$this->e("<link rel=\"shortcut icon\" href=\"".$this->ico."favicon.png\" />",$this->nivel + 1);
		...
		if(isset($_ENVIRONMENT)){
			if($_ENVIRONMENT===true){
				if($page->session->getSession("ENVIRONMENT_NO_FORCED")!="1"){
