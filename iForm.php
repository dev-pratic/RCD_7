...
class iForm extends generic {
	...
	public function setDataSet($dataSet,$table,$listName=false,$panel=false,$filial=true){
		$this->dataSet = $dataSet;
		$this->table = $table;
		$this->listName = $listName;
		$this->panel = $panel;
		$this->filial = $filial;
		$this->ins = new iInsert($this->dataSet, $this->table, $this->filial); 
		$this->upd = new iUpdate($this->dataSet, $this->table, $this->filial); 
	}
	public function autoBuild(){
		...
		$sel = new iSelect($this->dataSet,$this->table,$this->filial);
		$sel->listColumn();
		...
		if($resSel===false){
			...
			while ($row = $sel->read()){
				if($row['Field']==$fieldIdName && strlen($page->session->getSession($this->id,$PAGENAME))>0){
					# CARREGA OS DADOS
					$selB = new iSelect($this->dataSet,$this->table,$this->filial);
				...
				if(strpos($row['Field'],"estado")!==false){
					$selC = new iSelect($this->dataSet,"estado",$this->filial);
	                    ...
	                    if($name->formName==$this->name){
	                        ...
	                        if($name->getUnique()===true){
	                            if($this->dataSet->versao=="old"){
									...
									$this->sel = new iSelect($this->dataSet,$this->table,$this->filial);
