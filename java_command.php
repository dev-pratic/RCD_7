...
class command {
	...
	public function exeAnyEvent($event,$obj,$function,$execute=false,$id=false){
		$type = "exeAnyEvent".$obj.$id;

		$this->setFunction($type,"var obj".$obj.$id." = document.getElementById(\"".$obj."\");",$this->nivel);
		$this->setFunction($type,"obj".$obj.$id.".addEventListener(\"".$event."\", exeAnyEvent".$obj.$id.", true);",$this->nivel);
		$this->setFunction($type,"function exeAnyEvent".$obj.$id."() {",$this->nivel);
		$this->setFunction($type,$function,$this->nivel+1);
		$this->setFunction($type,"}",$this->nivel);
	}
	public function goToPageEvent($event,$obj,$pageDest="",$parameter="",$value=false,$target="",$addParameter="",$port="",$objToGetValue=false,$child=false,$execute=false,$timeout=false,$parameterB=false,$addParameterB=false,$objToGetValueB=false,$id=false){
		global $page;
		...
		if($objToGetValueB!=false){
			$this->setFunction($type,"var objToGetValueB".$objToGetValueB.$id." = document.getElementById(\"".$objToGetValueB."\");",$this->nivel);
		}
		if($timeout===false){
		...
		if(strtoupper($value)=="THIS"){
			if($target!='' && $target!='_parent' && $target!='_PARENT'){
				...
				if($parameter!=''){
					if($objToGetValue===false){
						$srcB = "'".$uri."' + '".$pageDest."' + '?' + '".$parameter."' + '=' + obj".$obj.$id.".value + '".$addParameter."'";
					}else{
						$srcB = "'".$uri."' + '".$pageDest."' + '?' + '".$parameter."' + '=' + objToGetValue".$objToGetValue.$id.".value + '".$addParameter."'";
					}
				}else{
					$srcB = "'".$uri."' + '".$pageDest."'";
				}
				if($parameterB!==false && $objToGetValueB!==false){
					$srcB .= " + '&".$parameterB."' + '=' + objToGetValueB".$objToGetValueB.$id.".value + '".$addParameterB."'";
				}
				$this->setFunction($type,"window.open(".$srcB.",'_blank');",$this->nivel+1);
			}else if($target=='_parent' || $target=='_PARENT'){
				if($parameter!=''){
					if($objToGetValue===false){
						$srcB = "'".$pageDest."' + '?' + '".$parameter."' + '=' + obj".$obj.$id.".value + '".$addParameter."'";
					}else{
						$srcB = "'".$pageDest."' + '?' + '".$parameter."' + '=' + objToGetValue".$objToGetValue.$id.".value + '".$addParameter."'";
					}
				}else{
					$srcB = "'".$pageDest."'";
				}
				...
				$this->setFunction($type,"window.parent.location.href = ".$srcB.";",$this->nivel+1);
			}else if($child!=false){
				if($pageDest!==false){
					if($parameter!=''){
						if($objToGetValue===false){
					...
					$this->setFunction($type,"document.getElementById('".$child."').src = ".$srcB.";",$this->nivel+1);
				}else{
					...
					if($parameter!=''){
						if($objToGetValue===false){
							$srcB = "'?' + '".$parameter."' + '=' + obj".$obj.$id.".value + '".$addParameter."'";
						}else{
							$srcB = "'?' + '".$parameter."' + '=' + objToGetValue".$objToGetValue.$id.".value + '".$addParameter."'";
						}
					}
					if($parameterB!==false && $objToGetValueB!==false){
						$srcB .= " + '&".$parameterB."' + '=' + objToGetValueB".$objToGetValueB.$id.".value + '".$addParameterB."'";
					}
					$this->setFunction($type,"document.getElementById('".$child."').src = srcTmp".$srcB.";",$this->nivel+1);
				}
			}else{
				if($parameter!=''){
					if($objToGetValue===false){
				...
				$this->setFunction($type,"document.location.href = ".$srcB.";",$this->nivel+1);
			}
		}else{
			if($target!='' && $target!='_parent' && $target!='_PARENT'){
				if($port){
				...
				if($parameter!=''){
					$this->setFunction($type,"window.open('".$uri."' + '".$pageDest."' + '?' + '".$parameter."' + '=' + '".$value."' + '".$addParameter."','_blank');",$this->nivel+1);
				}else{
					$this->setFunction($type,"window.open('".$uri."' + '".$pageDest."','_blank');",$this->nivel+1);
				}
			}else if($target=='_parent' || $target=='_PARENT'){
				if($parameter!=''){
				...
				if($pageDest!==false){
					if($parameter!=''){
						if($objToGetValue===false){
							$srcB = "'".$pageDest."' + '?' + '".$parameter."' + '=' + '".$value."' + '".$addParameter."'";
						}else{
							$srcB = "'".$pageDest."' + '?' + '".$parameter."' + '=' + objToGetValue".$objToGetValue.$id.".value + '".$addParameter."'";
						}
					}else{
					...
					if($parameter!=''){
						if($objToGetValue===false){
							$srcB = " + '?' + '".$parameter."' + '=' + '".$value."' + '".$addParameter."'";
						}else{
							$srcB = " + '?' + '".$parameter."' + '=' + objToGetValue".$objToGetValue.$id.".value + '".$addParameter."'";
						}
					}
	...
	public function goToPage(){
		...
		$this->setFunction($type,"}else if(Target!='' && Target!='_parent' && Target!='_PARENT'){",$this->nivel+1);
        	$this->setFunction($type,"if(Parameter!=''){",$this->nivel+2);
				$this->setFunction($type,"window.open('".$uri."' + Page + '?' + Parameter + '=' + Value + addParameter,'_blank');",$this->nivel+3);
        	$this->setFunction($type,"}else{",$this->nivel+2);
        		$this->setFunction($type,"window.open('".$uri."' + Page,'_blank');",$this->nivel+3);
       		$this->setFunction($type,"}",$this->nivel+2);
	...
	public function setValueEvent($event,$obj,$objAffected,$value,$innerHTML=false,$id=false){		
		...
		if($innerHTML===false){
			...
			$this->setFunction($type,"setValueAffected".$objAffected.$id.".innerHTML = value;",$this->nivel+1);
		}
		$this->setFunction($type,"}",$this->nivel);
	}
	...
	public function select($event="",$obj="",$objAffected="",$id=false){
		$type = "select".$obj.$id;
		$this->setFunction($type,"try{",$this->nivel);
		$this->setFunction($type,"var select".$obj.$id." = document.getElementById(\"".$obj."\");",$this->nivel+1);
		$this->setFunction($type,"select".$obj.$id.".addEventListener(\"".$event."\",function() { selectEvent".$obj.$id."(); }, true);",$this->nivel+1);
		$this->setFunction($type,"function selectEvent".$obj.$id."(){",$this->nivel+1);
		if($objAffected===false){
			$this->setFunction($type,"select".$obj.$id.".select();",$this->nivel+2);	
		}else{
			$this->setFunction($type,"var select".$objAffected.$id." = document.getElementById(\"".$objAffected."\");",$this->nivel+1);
			$this->setFunction($type,"select".$objAffected.$id.".select();",$this->nivel+2);	
		}
		$this->setFunction($type,"}",$this->nivel+1);
		$this->setFunction($type,"}catch(e){",$this->nivel);
		$this->setFunction($type,"alert('Erro: selectEvent - ' + e);",$this->nivel+1);
		$this->setFunction($type,"}\n",$this->nivel);
	}
	public function showMsg($event="",$obj="",$msg="",$pathCaseYes="",$pathCaseNo="",$typeButton="n",$objAffected=false,$target=false,$backGroundColor=COLOR_ULTRA_DARK_RED,$showComment=false,$formToSendComment=false,$objCore=false,$id=false){
		global $BASE_CARREGANDO_CONT;
		global $page;
		...
		if($pathCaseYes!=$pathCaseNo){
			$this->setFunction($type,"document.getElementById(\"btNo\").style.display = \"block\";",$this->nivel+2);
		}else{
			$this->setFunction($type,"document.getElementById(\"btNo\").style.display = \"none\";",$this->nivel+2);
		}
		...
		if($pathCaseYes=="" && $pathCaseYes!==false){
			...
			$this->setFunction($type,"goToPage('".$pathCaseYes."','','','','','".$objCore."');",$this->nivel+3);
		...
		if($pathCaseNo==""){
			...
			if($pathCaseNo=="" && $pathCaseNo!==false){
				...
				$this->setFunction($type,"goToPage('".$pathCaseNo."','','','','','".$objCore."');",$this->nivel+3);
	...
	public function core($event="",$obj="",$objToLoad="",$script="",$objToGetParameter="",$parameterName="",$pagename="",$addParameter=false,$timeout=false,$showLoading=true,$execute=true,$visible=false,$objToGetParameterAdd=false,$parameterNameAdd=false,$pagenameAdd=false,$id=false){
		global $BASE_CARREGANDO_CONT;
		global $page;
		...
		$objCore = new iframe($page, "objCore".$obj.$id,"objCore");
		if($execute===true){
			$objCore->src = $script;
		}
		...
		$menuCore->inSide("<div id=\"btViewCore".$obj.$id."\" name=\"btViewCore".$obj.$id."\" class=\"bigButton btn btn-secondary\" style=\"width:80%;text-align:center;margin-left:4%;margin-top:5px;\">Core ".$obj." ".$id."</div>");
		...
		if($objToGetParameterAdd!==false){
			$objToGetParameterAdd = explode(",",$objToGetParameterAdd);
		}else{
			$objToGetParameterAdd = array();
		}
		if($parameterNameAdd!==false){
			$parameterNameAdd = explode(",",$parameterNameAdd);
		}else{
			$parameterNameAdd = array();
		}
		if($pagenameAdd!==false){
			$pagenameAdd = explode(",",$pagenameAdd);
		}else{
			$pagenameAdd = array();
		}
		if(count($objToGetParameterAdd)>0){
			foreach($objToGetParameterAdd as $k => $v){
				$this->setFunction($type,"var coreObjGetParameterAdd".$v.$k.$id." = document.getElementById(\"".$v."\");",$this->nivel);
			}
		}
		...
		if(count($objToGetParameterAdd)>0){
			$this->setFunction($type,"var ScriptAdd = '';",$this->nivel+1);
			foreach($objToGetParameterAdd as $k => $v){
				$this->setFunction($type,"trataValor = coreObjGetParameterAdd".$v.$k.$id.".value;",$this->nivel+1);
				$this->setFunction($type,"coreObjGetParameterAdd".$v.$k.$id.".value = trataValor;",$this->nivel+1);
				if(count($pagenameAdd)==0){
					$this->setFunction($type,"ScriptAdd = ScriptAdd + '&".$parameterNameAdd[$k]."=' + trataValor;",$this->nivel+1);
				}else{
					$this->setFunction($type,"ScriptAdd = ScriptAdd + '&".$parameterNameAdd[$k]."=' + trataValor + '@".$pagenameAdd[$k]."';",$this->nivel+1);
				}
			}
		}else{
			$this->setFunction($type,"var ScriptAdd = '';",$this->nivel+1);
		...
		$this->setFunction($type,"objCore".$obj.$id.".src = Script + ScriptAdd;",$this->nivel+1);
	...
	public function submitFormEvent($event,$obj,$objAffected,$action,$target,$resetActionTarget,$loading=true){
		global $BASE_CARREGANDO_CONT;
	...
	public function setVisible($event="",$obj="",$objAffected="",$ns="",$as="block"){
		if($event==""){
			$type = "setVisible";
			$this->setFunction($type,"try{",$this->nivel);
			$this->setFunction($type,"function setVisible(Objeto){",$this->nivel+1);
			$this->setFunction($type,"document.getElementById(Objeto).style.display = '".$as."';",$this->nivel+2);
			$this->setFunction($type,"}",$this->nivel+1);
			$this->setFunction($type,"}catch(e){",$this->nivel);
			...
			$this->setFunction($type,"document.getElementById('".$objAffected."').style.display = '".$as."';",$this->nivel+2);
			$this->setFunction($type,"}",$this->nivel+1);
			$this->setFunction($type,"}catch(e){",$this->nivel);
	...
	public function setVisibleTogger($event="",$obj="",$objAffected="",$force=false,$ns="",$memory=false,$forcedDisplayNoneOnlyFirstTime=false){
		global $page;
		global $PAGENAME;
		if($event==""){
			...
			if($force!="flex"){
				...
				$this->setFunction($type,"if(document.getElementById('".$objAffected."').style.display=='flex'){",$this->nivel+1);
			}
			...
			if($force===false){
				...
				$this->setFunction($type,"}else if(document.getElementById('".$objAffected."').style.display=='none'){",$this->nivel+1);
				$this->setFunction($type,"document.getElementById('".$objAffected."').style.display = 'flex';",$this->nivel+2);
				$this->setFunction($type,"}else{",$this->nivel+1);
				$this->setFunction($type,"document.getElementById('".$objAffected."').style.display = 'none';",$this->nivel+2);
				$this->setFunction($type,"}",$this->nivel+1);
			}
			if($memory===true){
				$clearCardMemory = false;
				if($forcedDisplayNoneOnlyFirstTime===true){
					if(!$page->s->g("forcedDisplayNoneOnlyFirstTime".$obj)){
						$page->s->s("forcedDisplayNoneOnlyFirstTime".$obj,"1");
						$clearCardMemory = true;
					}
				}
				$this->setFunction($type,"if(typeof(Storage)!==\"undefined\"){",$this->nivel+1);
				$this->setFunction($type,"localStorage.setItem(\"cardMemory_".$obj."\",document.getElementById('".$objAffected."').style.display);",$this->nivel+2);		
				$this->setFunction($type,"}",$this->nivel+1);
				$this->setFunction($type,"}",$this->nivel);
				$this->setFunction($type,"if(typeof(Storage)!==\"undefined\"){",$this->nivel);
				$this->setFunction($type,"if(localStorage.getItem(\"cardMemory_".$obj."\")==\"none\"){",$this->nivel+1);		
				$this->setFunction($type,"document.getElementById('".$objAffected."').style.display = 'none';",$this->nivel+2);
				$this->setFunction($type,"}",$this->nivel+1);
				$this->setFunction($type,"function clearCardMemory_".$obj."(){",$this->nivel+1);
				$this->setFunction($type,"localStorage.setItem(\"cardMemory_".$obj."\",\"\");",$this->nivel+2);			
				$this->setFunction($type,"}",$this->nivel+1);
				if($clearCardMemory===true){
					$this->setFunction($type,"clearCardMemory_".$obj."();",$this->nivel+1);
					$this->setFunction($type,"if(typeof(Storage)!==\"undefined\"){",$this->nivel+1);
					$this->setFunction($type,"document.getElementById('".$objAffected."').style.display = 'none';",$this->nivel+2);	
					$this->setFunction($type,"localStorage.setItem(\"cardMemory_".$obj."\",document.getElementById('".$objAffected."').style.display);",$this->nivel+2);		
					$this->setFunction($type,"}",$this->nivel+1);
