...
function newUpload(obj,objLabel,objListaUpload,objPhpFileUpload,uploadConfig,resetList,uploadCompleted,msgErrorA){
	if(typeof uploadConfig === "undefined"){
		uploadConfig = uploadConfigStandard;
	}

	listObj.push(obj);
	listMimeTypes.push(uploadConfig.mime_types);

	if(typeof uploadCompleted === "undefined"){
	...
	try{
		var drop_ = document.getElementById(obj);
	...
	document.getElementById(obj).addEventListener('change', function() {
		try{
			...
			for(var i = 0; i < files.length; i++){
				validarArquivo(files[i],i,obj,objListaUpload,objPhpFileUpload,uploadConfig,uploadCompleted,msgErrorA);
			};
		}catch(e){
			alert("#2" + e);
		}
...
function validarArquivo(file,indice,obj,objListaUpload,objPhpFileUpload,uploadConfig,uploadCompleted,msgErrorA){
	try{	
		if(typeof msgErrorA === "undefined"){
			msgErrorA = "O arquivo " + file.name + " não permitido";
		}

		var mimeTypesTemp = false;
		for(var j = 0; j < listObj.length; j++){
			if(listObj[j]==obj){
				mimeTypesTemp = listMimeTypes[j];
			}
		}

		// Validar os tipos
		if(mimeTypesTemp.indexOf(file.type) == -1 && mimeTypesTemp.length>0) {
			aposValidar({"error" : msgErrorA},indice,obj,objListaUpload,objPhpFileUpload);
	        ...
	        img.onload = function(){
	            ...
	            if(result==true){
	            	aposValidar({"success": "Enviando: " + file.name},indice,obj,objListaUpload,objPhpFileUpload,uploadCompleted);
	            }else{
	            	aposValidar(erro,indice,obj,objListaUpload,objPhpFileUpload,uploadCompleted);
	            }
	        };
...
function aposValidar(info,indice,obj,objListaUpload,objPhpFileUpload,uploadCompleted){
	try{
		//Criar barra
		...
		if(info.error == undefined){
			...
			enviarArquivo(indice, barra,obj,objPhpFileUpload,uploadCompleted); //Enviar
		}else{
...
function enviarArquivo(indice, barra,obj,objPhpFileUpload,uploadCompleted){
	try{
