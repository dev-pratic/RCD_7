...
class java {
	...
	public function setObjVisible($event,$obj,$objAffected,$as="block"){
		$this->obj = new command();
		$this->obj->setVisible($event,$obj,$objAffected,count($this->arrayObj),$as);
	...
	public function setObjVisibleTogger($event,$obj,$objAffected,$force=false,$memory=false,$forcedDisplayNoneOnlyFirstTime=false){
		$this->obj = new command();
		$this->obj->setVisibleTogger($event,$obj,$objAffected,$force,count($this->arrayObj),$memory,$forcedDisplayNoneOnlyFirstTime);
		$this->arrayObj[count($this->arrayObj)] = $this->obj;
	}
	...
	public function exeAny($event,$obj,$function,$execute=false){
		if($event=="change"){
		...
		$this->obj->exeAnyEvent($event,$obj,$function,$execute);
	}
	public function setGoToPage($event,$obj,$page="",$parameter="",$value=false,$target="",$addParameter="",$port="",$objToGetValue=false,$child=false,$execute=false,$timeout=false,$parameterB=false,$addParameterB=false,$objToGetValueB=false){
		if($port!==false){
		...
		if(($this->stringToUpper($target)=="_BLANK" || $this->stringToUpper($target)=="BLANK") && $port!==false){
			$page = $_SERVER['SERVER_NAME'].$page;
		}
		$this->obj->goToPageEvent($event,$obj,$page,$parameter,$value,$target,$addParameter,$port,$objToGetValue,$child,$execute,$timeout,$parameterB,$addParameterB,$objToGetValueB,$id);
	}
	public function setFunctionGoToPage($pageDest="",$parameter="",$value=false,$target="",$addParameter="",$addPageColumn=false,$child=false,$cript=false){
		if($pageDest==""){
			$pageDest = $_SERVER['SCRIPT_NAME'];
		}
		if($addPageColumn===true){
			$pageDest = $pageDest."@varA";
		}
		if($value!==false && $value!="" && $cript===true){
			global $page;
			$value = $page->profiableEncode($value);
		}
        if($value!==false && $value!=""){
		    $this->lineCommand .= "goToPage('".$pageDest."','".$parameter."','".$value."','".$target."','".$addParameter."','".$child."');";
        }else if($value!==false){
        	$this->lineCommand .= "goToPage('".$pageDest."','".$parameter."','','".$target."','".$addParameter."','".$child."');";
        }else{
        	$addParameter = $addParameter.$addPageColumn;
            $this->lineCommand .= "goToPage('".$pageDest."','".$parameter."','".($cript===false ? "@varA" : "@varAcript")."','".$target."','".$addParameter."','".$child."');";
        }
		$this->obj->goToPage();
	}
	...
	public function setValue($event,$obj,$objAffected,$value,$innerHTML=false){
		$this->obj = $this->$event;
		$this->obj->setValueEvent($event,$obj,$objAffected,$value,$innerHTML);
	}
	...
	public function core($event,$obj,$objToLoad,$script,$objToGetParameter="",$parameterName="parameter",$pagename="",$addParameter=false,$timeout=false,$showLoading=true,$execute=true,$visible=false,$objToGetParameterAdd=false,$parameterNameAdd=false,$pagenameAdd=false,$forceNewId=false){
		$id = false;
		if($event!==false){
			if($event=="change"){
				$id = count($this->change);
		...
		$this->obj->core($event,$obj,$objToLoad,$script,$objToGetParameter,$parameterName,$pagename,$addParameter,$timeout,$showLoading,$execute,$visible,$objToGetParameterAdd,$parameterNameAdd,$pagenameAdd,$id);
	}
	public function select($event,$obj,$objAffected=false){
		$id = false;
		if($event=="click"){
			$id = count($this->clickArray);
			$this->clickArray[$id] = new command();
		...
		$this->obj->select($event,$obj,$objAffected,$id);
	}
	public function showMsg($event,$obj,$msg="Nenhuma Mensagem!",$pathCaseYes="",$pathCaseNo="",$type="n",$objAffected=false,$target=false,$backGroundColor=COLOR_ULTRA_DARK_RED,$showComment=false,$formToSendComment=false,$objCore=false){
		$id = false;
		if($event=="click"){
			$id = count($this->clickArray);
			$this->clickArray[$id] = new command();
		...
		$this->obj->showMsg($event,$obj,$msg,$pathCaseYes,$pathCaseNo,$type,$objAffected,$target,$backGroundColor,$showComment,$formToSendComment,$objCore,$id);
	}
	...
	public function setSubmitForm($event,$obj,$objAffected,$action="",$target="",$resetActionTarget=false,$loading=true){
		$this->obj = $this->click;
		$this->obj->submitFormEvent($event, $obj, $objAffected, $action, $target, $resetActionTarget, $loading);
	}
	...
	public function getLineCommand($varA="",$varB="",$dinamicVar=false,$repeatLineComand=false){
		if($this->lineCommand!=""){
	        ...
	        if($dinamicVar===false){
				if($varA!=""){ 
					if(strpos($this->lineCommand,"@varAcript")===false){
						$lineCommand = str_replace("@varA",$varA,$this->lineCommand."\""); 
					}else{
						global $page;

						$lineCommand = str_replace("@varAcript",$page->profiableEncode($varA),$this->lineCommand."\""); 
					}
				}
				if($varB!=""){
					$lineCommand = str_replace("@varB",$varB,$this->lineCommand."\"");
					if(strpos($this->lineCommand,"@varBcript")===false){
						$lineCommand = str_replace("@varB",$varB,$this->lineCommand."\""); 
					}else{
						global $page;

						$lineCommand = str_replace("@varBcript",$page->profiableEncode($varB),$this->lineCommand."\""); 
					}
				}
	        }else{
